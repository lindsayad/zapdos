...
validParams<Gas>()
{
  ...
  params.addParam<bool>("use_efield",
                        false,
                        "Whether to use the electric field instead of the electron mean energy as "
                        "the independent interpolation variable.");
  return params;
}
...
Gas::Gas(const InputParameters & parameters)
  : Material(parameters),
    ...
    _use_efield(getParam<bool>("use_efield"))
{
  ...
  std::vector<Real> electric_field;
  ...
  if (myfile.is_open())
  {
    while (myfile >> value)
    {
      if (_use_efield)
      {
        electric_field.push_back(value);
        myfile >> value;
      }
  ...
  if (_use_efield)
  {
    _alpha_interpolation.setData(electric_field, alpha);
    _alphaEx_interpolation.setData(electric_field, alphaEx);
...
Gas::computeQpProperties()
{
  ...
  if (!_use_efield)
  {
    if (_interp_trans_coeffs)
    {
      ...
      {
        _muem[_qp] =
            _mu_interpolation.sample(std::exp(_mean_en[_qp] - _em[_qp])) * _voltage_scaling;
        _d_muem_d_actual_mean_en[_qp] =
            _mu_interpolation.sampleDerivative(std::exp(_mean_en[_qp] - _em[_qp])) *
            _voltage_scaling;
